...
$("body").on("click", "#wzd_js_fn_01", function() {
		$.ajax({
			headers: {
			...
			error: function(respuesta){ alerta('Alerta!','Error de conectividad de red WZD-01');}
		});
});

$("body").on("click", "#wzd_js_fn_02", function() {
	var msj_error="";
		...
		dataType: 'json',
			success: function(resp_success){
				...
				if(resp_success['success']){
					$('#wzd_js_fn_03').prop('disabled', false);
				}
			},
...
$("body").on("click", "#wzd_js_fn_03", function() {
	var msj_error="";
	...
	if( $("#id_poliza").val() == "")	msj_error='Error de en el modulo de alta clave de error WZD-04';
	...
	$.ajax({
		...
		dataType: 'json',
			success: function(resp_success){
				if(resp_success['success']){
					$('#wzd_js_fn_03').prop('disabled', true);
					$('#wzd_js_fn_02').prop('disabled', true);
		...
		error: function(respuesta){ alerta('Alerta!','Error de conectividad de red WZD-03');}
	});
