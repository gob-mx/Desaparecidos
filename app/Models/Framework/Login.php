...
class Login extends Model
{
  ...
  static function caducarToken($token){
    DB::table('fa_tokens')
  ...
  static function logearConToken($token){

    $isseToken = DB::table('fa_tokens')
              ->select('id_token')
              ->where('url_token', '=', $token)
              ->where('cat_status_token', '=', 29)
              ->get();

    if(isset($isseToken[0])){

      $logged = DB::table('fa_tokens as fat')
                ->join('fa_remoteUser AS far', 'far.id_remoteUser', '=', 'fat.id_remoteUser')
                ->join('fw_usuarios AS fwu', 'far.id_usuario', '=', 'fwu.id_usuario')
                ->join('fw_usuarios_config AS fwuc', 'fwu.id_usuario', '=', 'fwuc.id_usuario')
                ->select('fwu.id_usuario', 'fwu.id_rol', 'fwu.usuario', 'fwu.id_ubicacion', 'fwu.correo', 'fwuc.aceptar_tyc', 'fwu.cat_pass_chge')
                ->where('fat.url_token', '=', $token)
                ->where('fat.cat_status_token', '=', 29)
                ->get();
